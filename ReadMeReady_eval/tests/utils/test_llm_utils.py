...
from ReadMeReady_eval.readme_ready.utils.llm_utils import (
...
def test_get_gemma_chat_model_with_peft():
    ...
    with patch("sys.platform", "linux"), patch(
        "ReadMeReady_eval.readme_ready.utils.llm_utils.hf_hub_download"
    ) as mock_hf_download, patch(
        "ReadMeReady_eval.readme_ready.utils.llm_utils.get_tokenizer"
    ) as mock_get_tokenizer, patch(
        "ReadMeReady_eval.readme_ready.utils.llm_utils.AutoModelForCausalLM.from_pretrained"
    ) as mock_auto_model, patch(
        "ReadMeReady_eval.readme_ready.utils.llm_utils.PeftModel.from_pretrained"
    ) as mock_peft_model, patch(
        "ReadMeReady_eval.readme_ready.utils.llm_utils.pipeline"
    ) as mock_pipeline, patch(
        "ReadMeReady_eval.readme_ready.utils.llm_utils.HuggingFacePipeline"
...
def test_get_tokenizer_with_hf_token(monkeypatch):
    ...
    gguf_file = "some_file.gguf"
    with patch(
        "ReadMeReady_eval.readme_ready.utils.llm_utils.AutoTokenizer.from_pretrained"
...
def test_get_embeddings_llama_model():
    ...
    with patch(
        "ReadMeReady_eval.readme_ready.utils.llm_utils.HuggingFaceEmbeddings"
...
def test_get_embeddings_non_llama_model():
    ...
    with patch(
        "ReadMeReady_eval.readme_ready.utils.llm_utils.OpenAIEmbeddings"
