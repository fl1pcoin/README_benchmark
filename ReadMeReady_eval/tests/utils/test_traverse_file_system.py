...
from ReadMeReady_eval.readme_ready.utils.traverse_file_system import traverse_file_system
...
def test_input_path_does_not_exist(capsys):
    with patch("ReadMeReady_eval.readme_ready.utils.traverse_file_system.Path") as MockPath:
...
def test_process_file_called_for_text_file():
    with patch("ReadMeReady_eval.readme_ready.utils.traverse_file_system.Path") as MockPath:
        with patch(
            "ReadMeReady_eval.readme_ready.utils.traverse_file_system.open",
            mock_open(read_data="dummy data"),
        ) as mock_file:
            with patch(
                "ReadMeReady_eval.readme_ready.utils.traverse_file_system.magic.from_buffer"
            ) as mock_magic:
