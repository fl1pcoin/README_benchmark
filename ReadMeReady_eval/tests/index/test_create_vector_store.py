...
from ReadMeReady_eval.readme_ready.index.create_vector_store import (
    create_vector_store,
    RepoLoader,
    process_directory,
    process_file,
...
from ReadMeReady_eval.readme_ready.types import LLMModels
...
def test_process_file():
    with patch("builtins.open", mock_open(read_data="file contents")):
        with patch(
            "ReadMeReady_eval.readme_ready.index.create_vector_store.should_ignore",
            return_value=False,
        ):
...
def test_repo_loader_load():
    with patch(
        "ReadMeReady_eval.readme_ready.index.create_vector_store.process_directory",
...
def test_create_vector_store(tmp_path):
    ...
    with patch.object(RepoLoader, "load", return_value=raw_docs):
        ...
        with patch.object(
            ...
            with patch(
                "ReadMeReady_eval.readme_ready.index.create_vector_store.get_embeddings"
            ) as mock_get_embeddings:
                mock_get_embeddings.return_value = MagicMock()
                with patch(
                    "ReadMeReady_eval.readme_ready.index.create_vector_store.HNSWLib"
                ) as mock_hnswlib:
