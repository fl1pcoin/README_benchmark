...
```bash
python -m OSA.osa_tool.run --repository https://github.com/username/repo --generate-workflows
```
...
```bash
python -m OSA.osa_tool.run --repository https://github.com/username/repo \
  --generate-workflows \
  --include-tests \
  --include-black \
  --include-pep8 \
