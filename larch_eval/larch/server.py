...
import larch
from larch_eval.larch.context_creator import ContextCreatorName
from larch_eval.larch.utils.download import cached_model_download, larch_model_url
from larch_eval.larch.postprocessor import postprocess
from larch_eval.larch.request_models import Response, GenerationRequest, \
...
async def create_generation(request: GenerationRequest):
    ...
    if request.model not in larch_eval.larch.generator.AVAILABLE_MODELS:
        raise HTTPException(
            status_code=503,
            detail=f'Model "{request.model}" is currently not available.')

    generator = larch_eval.larch.generator.AVAILABLE_MODELS[request.model]
...
async def get_models():
    ...
    models = []
    for model_name in larch_eval.larch.generator.AVAILABLE_MODELS.keys():
...
def _startup_event():
    ...
    larch_eval.larch.generator.init_models(
        loaded_models, os.environ.get('OPENAI_API_KEY'), encoder_decoder_model_paths
    )
    logger.info(f'Loaded {list(larch_eval.larch.generator.AVAILABLE_MODELS.keys())}')
...
def cli(reload: bool, log_level: str, workers: int, port: int, host: str):
    ...
    """
    uvicorn.run(
        'larch_eval.larch.server:app', host=host, port=port, reload=reload, log_level=log_level, workers=workers)
