...
def train(input: str, output: str, seed: int):
    dataset = larch_eval.larch.entrypoint_extractor.load_data(input)
    features = [
        file_features
        for repo_features in dataset.values()
        for file_features in repo_features.values()
    ]
    model = larch_eval.larch.entrypoint_extractor.train(features, seed=seed)
...
def predict(model: str, input: str):
    dir_tree = larch_eval.larch.context_creator.Directory.from_directory(input)
    path_to_features, _ = larch_eval.larch.entrypoint_extractor.load_single_data(
    ...
    best_path, _ = larch_eval.larch.entrypoint_extractor.predict(model_, path_to_features)
...
def predict_batch(model: str, input: str):
    ...
    for repo_name, path_to_features in dataset.items():
        if len(path_to_features) == 0:
            ...
            continue
        best_path, probs = larch_eval.larch.entrypoint_extractor.predict(model_, path_to_features)
